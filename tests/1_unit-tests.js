...
suite('UnitTests', () => {
    suite("solver tests", function () {
        ...
        test("Valid puzzle strings pass the solver", function (done) {
            assert.equal(
                solver.solve(validPuzzle), "135762984946381257728459613694517832812936745357824196473298561581673429269145378");
            done();
        });

        test("Invalid puzzle strings fail the solver", function (done) {
            let inValidPuzzle = "1.5..2.84..63.12.7.2..5..g..9..1....8.2.3674.3.7.2..9.47...8..1..16....926914.37.";
            assert.equal(solver.checkRegionPlacement(inValidPuzzle, "A", "2", "1"), false);
            done();
        });

        test("Solver returns the expected solution for an imcomplete puzzle", function (done) {
